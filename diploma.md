...
На третьем этапе проводилась предобработка собранных текстов. Предобработка включала нормализацию текста (приведение к нижнему регистру, удаление специальных символов), токенизацию (разбиение на слова) и базовую очистку данных (удаление пустых строк, нормализация пробелов). Для корректного анализа ключевых слов заголовки (Title) исключались из текста, а маркер "Abstract:" удалялся из начала аннотации. Каждый документ был сохранен в формате, удобном для последующей обработки.
...
Извлечение ключевых слов осуществлялось с использованием трех методов: TF-IDF n-граммы, YAKE и TextRank. Каждый метод применялся как к авторским, так и к синтетическим текстам для последующего сравнения и анализа различий. Для повышения корректности сравнения были реализованы:
- лемматизация ключевых слов (объединение словоформ, например, model/models → model; instruction/instructions → instruction);
- фильтрация служебных/метаданных терминов ("Abstract", "Title" и их производных) из наборов ключевых слов.
...
Сравнение результатов извлечения ключевых слов между авторскими и синтетическими текстами показало умеренные различия. После лемматизации и фильтрации метрики пересечения (Jaccard Index, Overlap Human/Synthetic, Harmonic Mean) по объединённой выборке (100 HUMAN vs синтетические от всех моделей) составили:
- NGRAMS: Jaccard=0.155, Overlap H=0.205, Overlap S=0.388, Harmonic=0.268;
- YAKE: Jaccard=0.026, Overlap H=0.033, Overlap S=0.116, Harmonic=0.051;
- TextRank: Jaccard=0.179, Overlap H=0.230, Overlap S=0.450, Harmonic=0.304.
Лемматизация уменьшила количество уникальных ключевых слов на 10–15% (например, для NGRAMS: HUMAN 837→748), что повысило корректность сопоставления наборов. Наблюдается асимметрия покрытия (Overlap Synthetic > Overlap Human), указывающая на большее число уникальных терминов в синтетических текстах. По степени различий (минимум перекрытия лучше) методы ранжируются так: YAKE (наименьшее перекрытие) > TF‑IDF n‑граммы > TextRank (наибольшее перекрытие).
...
Анализ частоты дискурсивных коннекторов показал, что при увеличении выборки различия становятся более заметными: HUMAN=3.68, AI=4.86 на 1000 слов (для используемого словаря), при этом Connectives AUC=0.642 указывает на умеренную разделимость классов по этому признаку; признак полезен как компонент ансамбля.
...
Визуализация комплексного анализа:

![Комплексный обзор метрик](results/additional_analysis/comprehensive_analysis.png)

![Сравнение моделей синтетики](results/additional_analysis/model_comparison.png)

![Корреляции между признаками](results/additional_analysis/correlation_heatmap.png)

### 2.5. Анализ различий ключевых слов (КС) в синтетических и авторских текстах

В этом подпункте фиксируются различия в наборах ключевых слов между HUMAN и AI без обучения моделей. Ключевые слова извлекаются методами TF‑IDF n‑грамм, YAKE и TextRank; тексты нормализуются, заголовки исключаются, маркер "Abstract:" удаляется, а ключевые слова лемматизируются для объединения словоформ. Далее формируются топ‑K списки по классам при K ∈ {5, 10, 25, 40, 50} и вычисляются метрики схожести/различий.

- Метрики:
  - Jaccard Index [14]: доля пересечения относительно объединения множеств (0 — нет общих КС, 1 — идентичные наборы).
  - Overlap Human / Overlap Synthetic: доли пересечения относительно каждого набора (асимметричные метрики).
  - Harmonic Mean (F1@K): гармоническое среднее Overlap Human и Overlap Synthetic; чувствительно к балансу пересечения.

- Интерпретация:
  - Низкий Jaccard при малых K указывает на специализированные, слабо совпадающие топ‑термины классов.
  - Асимметрия Overlap (часто Overlap Synthetic > Overlap Human) означает, что AI‑списки включают более общие термины, тогда как HUMAN‑списки сохраняют предметно‑специфичные маркеры.
  - Рост K обычно повышает Jaccard/Overlap/F1@K за счёт «длинного хвоста» общеупотребимых терминов.

- Наблюдения по методам:
- TextRank даёт наибольшее сбалансированное перекрытие (высокий F1@K) и умеренно высокий Jaccard; YAKE — минимальное перекрытие (низкие Jaccard и F1@K); TF‑IDF n‑граммы занимают промежуточное положение.
  - YAKE даёт более общие фразы и, как правило, более низкие значения F1@K.
  - TF‑IDF n‑граммы занимают промежуточное положение; результаты зависят от диапазона n и частотных порогов.

- Представление результатов:
  - Таблицы топ‑K для HUMAN и AI (по каждому методу и K) и списки «discriminative» КС (отличающие слова) с эффект‑сайзом.
  - Диаграммы Venn/UpSet и тепловые карты Jaccard/Overlap по методам и K.
  - Бар‑чарты топ отличающих КС с пометками направления (HUMAN‑типичные vs AI‑типичные).

Эти выводы используются далее для построения признакового пространства: наличие/отсутствие отобранных КС становится бинарными признаками для классификации документов.

Итоги Эксперимента 1 (объединённая выборка, после лемматизации/фильтрации):

| Метод | Jaccard | Overlap H | Overlap S | Harmonic (F1@K) |
|-------|---------|-----------|-----------|------------------|
| NGRAMS | 0.155 | 0.205 | 0.388 | 0.268 |
| YAKE | 0.026 | 0.033 | 0.116 | 0.051 |
| TextRank | 0.179 | 0.230 | 0.450 | 0.304 |

Дополнительные характеристики наборов ключевых слов (после лемматизации):

| Метод | HUMAN уникальных | AI уникальных |
|-------|-------------------|---------------|
| NGRAMS | 748 | 394 |
| YAKE | 1515 | 431 |
| TextRank | 914 | 467 |

Дискурсивные коннекторы (на 1000 слов):

| Показатель | HUMAN | AI | AUC |
|------------|-------|----|-----|
| Connectives | 3.68 | 4.86 | 0.642 |

Ключевые выводы:
- Низкий Jaccard у YAKE (0.026) фиксирует наименьшее пересечение списков HUMAN/AI, что полезно для разделения классов по КС.
- Наибольший F1@K у TextRank (0.304) означает более сбалансированное перекрытие топ‑K в обоих направлениях; метод извлекает термины, релевантные обоим классам.
- TF‑IDF n‑граммы дают умеренную раздельность (J=0.155, F1@K=0.268) и хорошо работают как источник признаков для классификаторов.
- Перекрытие асимметрично (Overlap S > Overlap H): синтетические тексты содержат больше уникальных терминов, не встречающихся в HUMAN.

Визуализация результатов по ключевым словам (Эксперимент 1):

![Сравнение метрик Jaccard/Overlap/Harmonic по методам](results/experiment1/metrics_comparison.png)

![Топ ключевых слов по методам и классам](results/experiment1/top_keywords_comparison.png)

![Разнообразие ключевых слов](results/experiment1/diversity_analysis.png)

![Распределение частоты коннекторов](results/experiment1/connectives_distribution.png)

![Средние значения коннекторов](results/experiment1/connectives_bar.png)

### 2.6. Разработка методов детекции

Разработка методов детекции включала реализацию трех основных подходов: детекция на основе лексико-стилистических признаков, детекция с использованием семантических эмбеддингов и детекция на основе классификации по ключевым словам. Каждый подход был реализован с использованием различных алгоритмов машинного обучения и валидационных процедур. В качестве признаков для лексического канала использованы обновленные наборы КС после лемматизации/фильтрации.

Детекция на основе лексико-стилистических признаков включала использование метрик пересечения ключевых слов (Jaccard Index, Overlap Human/Synthetic, Harmonic Mean) и частоты дискурсивных коннекторов для разделения классов. Метод использовал пороговые значения для классификации документов на основе вычисленных метрик. Результаты показали умеренную разделимость классов: для коннекторов Connectives AUC=0.642, что позволяет использовать признак в составе ансамбля, но не как основной.
...
Визуализация результатов классификации на основе КС (Эксперимент 4):

![Accuracy в зависимости от K](results/experiment4/accuracy_by_k.png)

![ROC AUC в зависимости от K](results/experiment4/auc_by_k.png)

![Тепловая карта Accuracy по методам/классификаторам/K](results/experiment4/accuracy_heatmap.png)
...
Эксперимент 1 включал сравнение ключевых слов между 100 авторскими текстами (50 Text Mining + 50 Information Retrieval) и синтетическими текстами от различных LLM моделей (по 50 документов на тему от каждой модели) с использованием методов TF-IDF n-граммы, YAKE и TextRank. Для повышения точности сравнения была применена лемматизация ключевых слов (объединение словоформ) и исключение служебных терминов ("Abstract", "Title"). Метрики пересечения после обновлений: NGRAMS — Jaccard=0.155, Overlap H=0.205, Overlap S=0.388, Harmonic=0.268; YAKE — Jaccard=0.026, Overlap H=0.033, Overlap S=0.116, Harmonic=0.051; TextRank — Jaccard=0.179, Overlap H=0.230, Overlap S=0.450, Harmonic=0.304. Анализ частоты дискурсивных коннекторов показал умеренную разделимость классов (Connectives AUC=0.642). Дополнительные метрики (TTR, Self-BLEU-1, Gzip ratio) подтвердили, что синтетические тексты менее разнообразны лексически и более однотипны/повторяемы.
...
### 3.3. Сравнение с существующими подходами и соотнесение результатов

В научной литературе предложено несколько направлений решения задачи детекции синтетических текстов. Ниже приводится обзор ключевых подходов и сопоставление наших результатов с опубликованными работами.

#### 3.3.1. Основные направления в литературе

- Статистические/перплекситей‑подходы и вероятностные признаки. GLTR [11] визуализирует «аномальность» токенов по вероятностям языковой модели; подобные методы эффективны для ранних генераторов, однако чувствительны к перефразированию и доменной смене, а также требуют доступа к распределениям LLM. Смежные признаки: длина предложений, распределения частот, TTR, Self‑BLEU, Gzip‑ratio [35, 24]. 
- Лексико‑стилистические признаки. Работы [22, 33, 40] показывают, что связность, риторические структуры и дискурсивные коннекторы различаются у HUMAN и AI. Эти признаки интерпретируемы, но дают умеренную разделимость и подвержены доменным эффектам.
- Семантические эмбеддинги и линейные (или простые) классификаторы. Линия работ [31, 28, 41, 36] демонстрирует высокую эффективность: текст переводится в эмбеддинг фиксированного размера, далее обучаются логистическая регрессия, LinearSVM или MLP. Подход устойчив к перефразированию и хорошо переносится между источниками.
- Инструктивные/контекстные методы с противниками. Outfox [16] использует in‑context learning с «адверсариальными» примерами и показывает state‑of‑the‑art на эссе; цена — зависимость от конкретной LLM и вычислительная дороговизна при применении.
- Ключевые слова/ключевые фразы. Класс экстрактивных методов (TF‑IDF n‑граммы, RAKE/YAKE, TextRank, Topic/PositionRank [6, 13, 21, 3, 10]) традиционно применялся для суммаризации/поиска; в детекции используется реже. Преимущества — интерпретируемость и дешевизна, недостатки — чувствительность к нормализации и покрытию домена.

#### 3.3.2. Табличное сопоставление подходов

| Подход | Репрезентативные источники | Сильные стороны | Ограничения | Наши наблюдения/результаты |
|-------|-----------------------------|-----------------|-------------|----------------------------|
| Перплекситей/вероятностные признаки | GLTR [11], обзор [41] | Простота, визуальная интерпретация | Падает устойчивость на современных LLM и при перефразировании; нужен доступ к LM | Мы не делали перплекситей‑базовую ветку; близкие статистики (TTR, Self‑BLEU, Gzip) подтверждают стилевые различия, но ограниченно разделяют классы самостоятельно |
| Лексико‑стилистика (коннекторы, связность) | Mitchell et al. [22], RST [33], обзор [40] | Интерпретируемость, языконезависимость | Умеренная сила сигнала, доменная зависимость | Connectives AUC=0.642 (умеренная разделимость); полезно как вспомогательный канал |
| Семантические эмбеддинги + простые классификаторы | Solaiman et al. [31], SBERT [28], TURINGBENCH [36], обзор [41] | Высокая точность, устойчивость к перефразированию, переносимость | Требуются эмбеддинги и инференс модели; возможна чувствительность к стилю корпуса | Accuracy=1.000, AUC=1.000 на нашем корпусе (roberta‑base, albert‑base‑v2) |
| Экстракция ключевых слов и классификация по КС | YAKE [6], Hulth [13], TextRank [21], Topic/PositionRank [3, 10] | Интерпретируемость, малые требования к ресурсам | Чувствительно к нормализации; покрытие домена; n‑граммная «узость» | Новизна в нашем применении: после лемматизации/фильтрации признаки по КС дают AUC до 0.997 (N‑grams+RF, K=50) и Accuracy до 0.950 (K=40) |
| In‑context/адверсариальные методы | Outfox [16] | Высокое качество на целевых доменах | Стоимость, зависимость от конкретной LLM, воспроизводимость | Не применяли: компромисс в пользу оффлайн‑инференса и прозрачной репликации |

#### 3.3.3. Сопоставление количественных эффектов

- Семантическая ветка у нас даёт практически идеальные метрики (Accuracy/AUC≈1.000), что согласуется с выводами [31, 36, 41] о преимущественности эмбеддинговых признаков для современной генерации.  
- Лексико‑стилистика подтверждает тренд из [22, 40]: сигнал присутствует, но сам по себе умерен (у нас Connectives AUC=0.642). Практический вывод — использовать как дополнительный канал.  
- Ключевые слова, будучи редко применяемыми в детекции напрямую, после лемматизации/очистки показали конкурентные результаты: Accuracy=0.950 при K=40 (N‑grams+LR), AUC=0.989–0.997 в зависимости от комбинации. Это согласуется с идеей [13, 6, 21], что правильно нормализованные термины хорошо отражают тематическую «подпись» класса.

#### 3.3.4. Практические выводы по сравнению

1) Для высокоточного решения в реальной системе целесообразно ставить семантический детектор (эмбеддинги + линейный классификатор) как основной, что согласуется с [31, 36, 41].  
2) Для интерпретации и дешёвого скрининга — канал по ключевым словам (наш результат AUC≈0.99) с лемматизацией и фильтрацией метаданных; он прозрачен, быстро воспроизводим и объясним.  
3) Лексико‑стилистические признаки укрепляют ансамбль и повышают устойчивость к сдвигам домена, но не заменяют основные каналы.  
4) Адверсариальные и ICL‑подходы [16] перспективны для онлайновых сценариев, но в академическом контексте с ограничениями по бюджету/конфиденциальности предпочтительна оффлайн‑архитектура (как в нашей работе).
