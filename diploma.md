...
В этом подпункте фиксируются различия в наборах ключевых слов между HUMAN и AI без обучения моделей. Ключевые слова извлекаются методами TF‑IDF n‑грамм, YAKE и TextRank; тексты нормализуются и лемматизируются для объединения словоформ. Далее формируются топ‑K списки по классам при K ∈ {5, 10, 25, 40, 50} и вычисляются метрики схожести/различий.
