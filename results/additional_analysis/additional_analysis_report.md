...
## Анализ различий по ключевым словам (Эксперимент 1)

В рамках Эксперимента 1 мы сравнили наборы ключевых слов, извлечённых методами N‑грамм (TF‑IDF), YAKE и TextRank, между HUMAN и AI. Для оценки различий использованы Jaccard, Overlap Human, Overlap Synthetic и их гармоническое среднее (F1@K).

- Сводные метрики:

| Метод   | Jaccard | Overlap Human | Overlap Synthetic | Harmonic Mean |
|---------|---------|----------------|-------------------|---------------|
| NGRAMS  | 0.106   | 0.123          | 0.433             | 0.191         |
| YAKE    | 0.060   | 0.071          | 0.274             | 0.113         |
| TextRank| 0.142   | 0.187          | 0.372             | 0.249         |

- Интерпретация:
  - TextRank показывает наибольшую разделимость: выше Jaccard и F1@K, чем у NGRAMS/YAKE — графовый метод лучше подхватывает контекстные связи.
  - Асимметрия Overlap: Overlap Synthetic > Overlap Human у всех методов. Это означает, что топ‑AI список включает больше терминов из человеческого словаря, чем наоборот; человеческие топ‑слова чаще остаются класс‑специфичными.
  - Различия в длине ключевых единиц: средняя длина у YAKE (~2.3 токена) выше, чем у NGRAMS (~1.43) и TextRank (~1.09), что влияет на семантическую «общность» терминов и пересечения.
  - Связующие (connectives) как отдельный признак дают слабую разделимость (AUC ≈ 0.533, Accuracy@threshold ≈ 0.45) и сами по себе не подходят для надёжной детекции.

- Наблюдения по распределениям:
  - HUMAN списки чаще содержат предметно‑специфичные термины домена, тогда как AI топ‑списки — более общие фразы (что и повышает Overlap Synthetic).
  - При увеличении K (в последующих экспериментах) метрики сходства обычно растут за счёт «длинного хвоста» общеупотребимых терминов — это важный эффект при интерпретации.

- Рекомендуемые визуализации (по имеющимся артефактам Эксп.1):
  - metrics_comparison.png — компактное сравнение Jaccard/Overlap/F1@K между методами;
  - top_keywords_comparison.png — иллюстрация различий в частотах топ‑КС между классами;
  - diversity_analysis.png — различия в длине и уникальности ключевых единиц по методам;
  - connectives_distribution.png и connectives_bar.png — распределение и средние по связующим;
  - top_keywords_text_mining.png и top_keywords_information_retrieval.png — примеры топ‑КС по тематикам.

Практически, эти различия подтверждают, что лексические маркеры (особенно из TextRank) могут служить информативными признаками для последующих моделей детекции. Далее мы используем выявленные различия при построении признакового пространства для классификации.
