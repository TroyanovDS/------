...
#### Применение для сравнения HUMAN и AI текстов

**Важно:** Self-BLEU-1 вычисляется **отдельно для каждого класса** (HUMAN и AI), а затем сравниваются полученные значения.

**Протокол сравнения:**
1. Вычисляем \(\mathrm{Self\text{-}BLEU\text{-}1}(C_{\mathrm{HUM}})\) для корпуса авторских текстов
2. Вычисляем \(\mathrm{Self\text{-}BLEU\text{-}1}(C_{\mathrm{AI}})\) для корпуса синтетических текстов
3. Сравниваем средние значения и распределения между классами

**Интерпретация различий:**
- Если \(\mathrm{Self\text{-}BLEU\text{-}1}(C_{\mathrm{AI}}) > \mathrm{Self\text{-}BLEU\text{-}1}(C_{\mathrm{HUM}})\) → синтетические тексты более похожи друг на друга, имеют меньшее лексическое разнообразие
- Если \(\mathrm{Self\text{-}BLEU\text{-}1}(C_{\mathrm{AI}}) < \mathrm{Self\text{-}BLEU\text{-}1}(C_{\mathrm{HUM}})\) → синтетические тексты более разнообразны (редко для AI-генерации)
- **Эмпирические наблюдения:** В нашем исследовании AI тексты показывают более высокий Self-BLEU-1 (0.849–0.880) по сравнению с HUMAN (0.783), что указывает на меньшую вариативность синтетических текстов

**Примечание о нормализации:**
- По умолчанию Self-BLEU-1 работает с точными совпадениями словоформ (токенов)
- В нашем исследовании применяется **лемматизация** (и удаление стоп-слов), поэтому метрика работает с **основами слов**, а не полными формами
- Это позволяет объединить разные грамматические формы одного слова (например, "model" и "models") и получить более точную оценку лексического разнообразия
...
### TF-IDF Coherence (локальная когерентность текста)

#### Определение

**TF-IDF Coherence** — метрика, измеряющая среднюю косинусную близость между соседними предложениями в тексте. Она отражает локальную смысловую согласованность и плавность переходов между последовательными частями документа.

#### Формула

Для документа \(d\), разбитого на предложения \(S = \{s_1, s_2, \ldots, s_{|S|}\}\), где каждое предложение \(s_i\) представлено TF-IDF вектором \(v_i\):

\[
\mathrm{Coherence}(d) = \frac{1}{|S| - 1} \sum_{i=1}^{|S|-1} \cos(v_i, v_{i+1})
\]

где:
- \(|S|\) — общее количество предложений в тексте
- \(v_i\) — TF-IDF вектор \(i\)-го предложения (униграммы и биграммы, стоп-слова удалены)
- \(\cos(v_i, v_{i+1}) = \frac{v_i \cdot v_{i+1}}{||v_i|| \cdot ||v_{i+1}||}\) — косинусное сходство между соседними предложениями

#### Протокол вычисления

1. **Токенизация**: Текст разбивается на предложения
2. **Векторизация**: Для каждого предложения создается TF-IDF вектор (ngram_range=(1,2), stop_words='english', max_features=4000)
3. **Вычисление сходства**: Для каждой пары соседних предложений вычисляется косинусное сходство
4. **Усреднение**: Берется среднее арифметическое всех значений сходства для документа

#### Агрегация по корпусу

\[
\overline{\mathrm{Coherence}}(C) = \frac{1}{|C|} \sum_{d \in C} \mathrm{Coherence}(d)
\]

где \(|C|\) — количество документов в корпусе.

#### Интерпретация значений

- **Диапазон**: [0, 1], где 0 — отсутствие сходства между соседними предложениями, 1 — полное сходство
- **Высокое значение** (ближе к 1) → более плавные смысловые переходы, лучшая локальная связность текста
- **Низкое значение** (ближе к 0) → более резкие переходы, фрагментированная структура

#### Связь с детекцией синтетических текстов

**Эмпирические наблюдения:**
- В нашем исследовании для научных текстов: HUMAN ≈ 0.036, AI ≈ 0.028 (среднее по моделям)
- Синтетические тексты показывают **несколько более низкую** локальную когерентность по сравнению с авторскими

**Характеристика текстов по значениям метрики:**

Оба корпуса демонстрируют низкие значения TF-IDF Coherence (0.02–0.04), что характерно для научных текстов с высокой лексической разнородностью: каждое предложение может вводить новые концепции и термины, что естественным образом приводит к ограниченному лексическому пересечению между последовательными частями текста. **Авторские тексты** (≈0.036) показывают несколько более высокую степень согласованности в использовании терминологии и более плавные переходы между предложениями, в то время как **синтетические тексты** (≈0.028) демонстрируют несколько более выраженную вариативность в распределении лексики между соседними предложениями. Следует отметить, что разница между классами незначительна, и оба типа текстов находятся в диапазоне значений, типичном для научных абстрактов, что указывает на схожие структурные характеристики на уровне локальной когерентности.

**Интерпретация различий:**
1. **Структурные особенности**: Авторские тексты демонстрируют более плавные переходы между предложениями, что может быть связано с более естественным развитием мысли
2. **Лексическое разнообразие**: Синтетические тексты могут иметь более резкие смены тематики или менее согласованное использование терминологии между соседними предложениями
3. **Ограничения метрики**: Абсолютные значения низкие (0.02–0.04) из-за разреженности TF-IDF векторов на уровне предложений; метрика чувствительна к сегментации предложений и параметрам векторизации

**Применение в детекции:**
- TF-IDF Coherence является **слабым признаком** для детекции (различия небольшие, ~0.008)
- Рекомендуется использовать как **вспомогательный компонент** в ансамбле признаков вместе с другими метриками (TTR, Self-BLEU, Connectives)
- Более эффективна для выявления фрагментированных или несвязных текстов, чем для надежного разделения HUMAN/AI

**Выводы из схожести значений между корпусами:**

Схожесть значений TF-IDF Coherence между HUMAN (≈0.036) и AI (≈0.028) корпусами указывает на то, что современные языковые модели достигли высокого уровня структурной связности, сопоставимого с человеческими текстами, поэтому локальная когерентность на уровне соседних предложений не является надежным дифференцирующим признаком для детекции и должна использоваться только как вспомогательный компонент в ансамбле с более информативными метриками (Self-BLEU, TTR, Connectives).

#### Обозначения
- \(d\) — документ; \(S\) — множество предложений; \(|S|\) — число предложений; \(v_i\) — TF-IDF вектор предложения; \(\cos(\cdot, \cdot)\) — косинусное сходство; \(C\) — корпус документов.

### Сопоставление подходов детекции и тактика применения

В научной литературе предложено несколько направлений решения задачи детекции синтетических текстов:

- **Лексико‑стилистические признаки.** В работе «Fast detection of machine-generated text» показывают, что связность, риторические структуры и дискурсивные коннекторы различаются у HUMAN и AI. Эти признаки интерпретируемы, но дают ограниченную разделимость.
- **Семантические эмбеддинги и классификаторы.** Ряд работ демонстрирует высокую эффективность: текст переводится в эмбеддинг фиксированного размера, далее обучаются модели (логистическая регрессия, многослойный персептрон). Подход устойчив к перефразированию и переносится между источниками. [12, 13, 14]
- **Ключевые слова/ключевые фразы.** Класс экстрактивных методов (TF‑IDF n‑граммы, RAKE/YAKE, TextRank, Topic/PositionRank) традиционно применялся для суммаризации/поиска; в детекции используется реже. Преимущества — интерпретируемость и низкая стоимость; недостатки — чувствительность к нормализации. [15]

Тактика применения по результатам нашего исследования и обзора литературы:

- Для высокоточного решения целесообразно использовать семантический детектор (эмбеддинги + классификатор) как основной канал.
- Для интерпретации и дешёвого скрининга — канал по ключевым словам с лемматизацией и фильтрацией метаданных; он прозрачен и воспроизводим.
- Лексико‑стилистические признаки усиливают ансамбль, но не заменяют основные каналы.
